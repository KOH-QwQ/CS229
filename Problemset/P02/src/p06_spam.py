...
def transform_text(messages, word_dictionary):
    ...
    """
    # *** START CODE HERE ***
    word_matrix = np.zeros((len(messages), len(word_dictionary)), dtype=int)
    for i in range(len(messages)):
        word_list=get_words(messages[i])
        for word in word_list:
            if word in word_dictionary:
                word_matrix[i][word_dictionary[word]] += 1
    return word_matrix
    # *** END CODE HERE ***
...
def fit_naive_bayes_model(matrix, labels):
    ...
    phi_k_y1 = (matrix_y1 + 1) / (matrix_y1.sum() + n)
    phi_k_y0 = (matrix_y0 + 1) / (matrix_y0.sum() + n)
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    sum1 = (matrix @ np.log(phi_k_y1)) + np.log(phi_y)
    sum0 = (matrix @ np.log(phi_k_y0)) + np.log(1 - phi_y)
    return (sum1>sum0).astype(int)
    # *** END CODE HERE ***
